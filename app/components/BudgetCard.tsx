...
export default function BudgetCard({ spent, total, onEdit }: any) {
  ...
  if (percentage > 75) colorClass = "bg-purple-400"
    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden">
      <div className="flex justify-between items-end mb-2 relative z-10">
        <div>
          ...
          <h2 className={`text-5xl font-bold tracking-tight ${remaining < 0 ? 'text-red-500' : 'text-gray-800'}`}>
            ₹{remaining}
          </h2>
        </div>
        <button onClick={onEdit} className="text-right group">
          <div className="flex items-center gap-1 justify-end text-xs text-gray-400 group-hover:text-purple-600 transition-colors">
            <span>Limit</span>
            <Edit2 size={12} />
          </div>
      ...
      <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden mt-6 relative z-10">
        <div className={`h-full rounded-full transition-all duration-1000 ease-out ${colorClass}`} style={{ width: `${percentage}%` }} />
      </div>
      <p className="text-xs text-gray-400 mt-2 text-right">
        {percentage.toFixed(1)}% used ({new Date().toLocaleString('default', { month: 'long' })})
      </p>
    </div>
  )
}